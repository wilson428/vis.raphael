...
var graph = function(opts, info, dataset) {
	var chrt = chart(opts, info, dataset),
	c,
	i,
	graph;

	//go up chain to determine graph type for each y set
	for (c = 0; c < info.datasets.length; c += 1) {
		if (typeof(info.yscales[info.datasets[c]].graph_type) === "undefined") {
			info.yscales[info.datasets[c]].graph_type = info.graph_type;
			if (typeof(info.yscales[info.datasets[c]].graph_type) === "undefined") {
				info.yscales[info.datasets[c]].graph_type = opts.graph_type;
				if (typeof(info.yscales[info.datasets[c]].graph_type) === "undefined") {
					info.yscales[info.datasets[c]].graph_type = "LINE";
				}
			}
		}
	}
	...
	if (typeof (opts.legend) === "undefined" || opts.legend) {
		...
		var c = 0;
		for (i in info.yscales) {
			if (info.yscales.hasOwnProperty(i)) {
				lx = opts.x + opts.padding.left + opts.legend_position[0] + Math.floor(c / 2) * opts.legend_position[2];
				ly = opts.y + opts.legend_position[1] + (c % 2) * opts.legend_position[3];
				//if color not specified, choose random
				if (typeof (info.yscales[i].color) === "undefined") {
					info.yscales[i].color = "rgb(" + randInt(256) + "," + randInt(256) + "," + randInt(256) + ")";
				}
				//draw a line for LINE graph, rect for FILL graph
				if (info.yscales[i].graph_type === "LINE") {
					chrt.get_paper().path("M " + (lx-20) + "," + ly + "L" + (lx-5) + "," + ly).attr({"stroke" : info.yscales[i].color, "stroke-width" : 3});
				} else {
					chrt.get_paper().rect(lx-15, ly-5, 10, 10).attr({"fill" : info.yscales[i].color, "stroke-opacity" : 0, "opacity" : 0.8});
				}
				chrt.get_paper().text(lx, ly, info.yscales[i].label).attr({"text-anchor" : "start", "font-family": "'Arial', serif", "font-size" : 12, "font-weight" : "bold"});
				c += 1;
			}
		}
	}
	...
	graph = function () {
		...
		for (i = 0; i < info.datasets.length; i += 1) {
			graph_type = info.yscales[info.datasets[i]].graph_type;
			if (graph_type === "LINE" || graph_type === "FILL") {
				...
				for (c = 0; c < info.values.length; c += 1) {
					...
					ay = Math.round(chart_info.yaxis.position.y-(info.values[c][info.datasets[i]]-info.yscales[info.datasets[i]].min) * chart_info.yaxis.scale);
					if (path === "") {
						path = 'M' + ax + ',' + ay;
					} else {
						path += 'L' + ax + ',' + ay;
			...
			if (graph_type === "FILL") {
				...
				path += 'L' + chart_info.xaxis.position.x + ',' + chart_info.xaxis.position.y - (info.values[0][info.datasets[i]]-chart_info.xaxis.position.min) * chart_info.xaxis.position.scale;
				attrs = { "fill" : info.yscales[info.datasets[i]].color, "opacity" : 0.8, "stroke" : info.yscales[info.datasets[i]].color, "stroke-width" : 2 };
			} else {
				attrs = { "stroke" : info.yscales[info.datasets[i]].color, "stroke-width" : 2, "opacity" : 1, "stroke-opacity" : 1};
			}
			
			shape = chrt.get_paper().path(path).attr(attrs);				
...
/*
				...
				//console.log(info.datasets[c] +" " + info.values[i][info.datasets[c]]);
			}
		}
	}
...
function () {
	function Chart(paper, instructions, data, opts) {
		...
		if (typeof (opts.legend) === "undefined" || opts.legend) {
			for (i in instructions.yscales) {
				if (instructions.yscales.hasOwnProperty(i)) {
					...
					lx = x + opts.padding.left + opts.legend_position[0] + Math.floor(c / 2) * opts.legend_position[2];
		...
		axis = function(data, info, dependency) {
			//positioning of axes
			var position = {
				x : x + opts.padding.left,
				y : y + height-opts.padding.bottom,
				top : y + opts.padding.top,
				right : x + width-opts.padding.right
			},
			ticks = paper.set(),
			labels = paper.set(),
			top,
